...
const server = http.createServer((req, res) => {
  ...
  if (pathname === '/' || pathname === '/overview') {
    ...
    const replacedTempCardHTML = jsonDataObj.map(el => replaceTemplate(tempCardHTML, el)).join();
    const output = tempOverviewHTML.replace(`{%PRODUCT_CARDS%}`, replacedTempCardHTML);
    res.end(output);
